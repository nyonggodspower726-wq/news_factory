...
class ArticleEngine:
    ...
    def _title(self,story,headline,synthesis):
        for value in (
    ...
    def _facts(self,package,synthesis,verification):
        raw=[]
        ...
        for item in raw:
            ...
            if status in {"CONTRADICTED","DISPUTED","UNVERIFIED","HOLD_FOR_REVIEW"}:continue
            seen.add(key)
            facts.append(text)
            if len(facts)>=self.max_facts:break
        return facts

    def _lead(self,title,facts,story,synthesis):
        if facts:
            first=facts[0]
            return f"{first}"
    ...
    def _questions(self,synthesis,story):
        values=[]
        for source,key in ((synthesis,"unknowns"),(synthesis,"questions"),(story,"reader_questions")):
            value=source.get(key) if isinstance(source,dict) else None
            if isinstance(value,list):values+=value
            elif value:values.append(value)
        return self._unique_text(values)[:6]

    def _sections(self,facts,context,consequences,next_steps):
        sections=[]
        if facts:sections.append({"heading":"What happened","content":facts})
        if context:sections.append({"heading":"Context","content":context})
        if consequences:sections.append({"heading":"Why it matters","content":consequences})
        if next_steps:sections.append({"heading":"What happens next","content":next_steps})
    ...
    def _category(self,story,synthesis,topic):
        value=self._text(story.get("category",story.get("story_type",synthesis.get("story_type","general"))))
        if value:return value.lower().replace(" ","-")
        return "general"

    def _tags(self,package,topic):
        values=[]
        if topic:values.extend(re.findall(r"\b[a-zA-Z][a-zA-Z0-9'-]{2,}\b",topic.lower()))
    ...
    def _source_links(self,package):
        sources=package.get("sources",[]) if isinstance(package,dict) else []
        output=[]
        if not isinstance(sources,list):return output
        for source in sources:
            if not isinstance(source,dict):continue
            url=self._text(source.get("url"))
            if not url:continue
            output.append({
    ...
    def _publication_safe(self,package):
        ...
        if isinstance(editorial,dict) and editorial.get("publication_gate") is False:
            return False
        status=self._text(verification.get("publication_status","")) if isinstance(verification,dict) else ""
    ...
    def _text(self,value):
        ...
        if isinstance(value,list):
            return self._clean_text(" ".join(str(x) for x in value))
...
def create_article(package):
    return article_engine.create(package)

def create_article_plan(package):
    return article_engine.create_article_plan(package)

if __name__=="__main__":
    test={
        "story":{"title":"Officials announce a new development","summary":"Officials announced a new development."},
        "synthesis":{"confirmed_facts":["Officials announced a new development."]},
        "verification":{},
