...
class JournalistEngine:
    def __init__(self,ai=None):
        ...
        self.min_words=int(os.getenv("JOURNALIST_MIN_WORDS","300"))
    ...
    def write(self,article_plan=None,story=None,sources=None,claims=None,evidence=None,verification=None,synthesis=None,story_model=None,narrative=None,angles=None,psychology=None,reader_psychology=None,engagement=None,significance=None,ai=None,**kwargs):
        ...
        try:
            ...
            attempts=0
            while self._word_count(article.get("content","")) < self.min_words and attempts<2:
                attempts+=1
                result=self._expand_if_needed(client,research,result,attempts)
                article=self._clean_article(result,research)
    ...
    def _research(self,article_plan,story,sources,claims,evidence,verification,synthesis,story_model,narrative,angles,psychology,reader_psychology,engagement,significance):
        ...
        for s in sources:
            ...
            stitle=self._text(s.get("title") or s.get("headline"))
            if url or text: usable.append({"name":name,"url":url,"title":stitle,"text":text[:10000]})
        facts=[]
        ...
        usable=self._relevant_sources(usable,title,story,facts)
        context=[]
...
Use ONLY the supplied research. Never invent facts, names, quotes, statistics, dates, motives, locations, analysts, experts, officials, reactions, or events. Never manufacture quotations or unnamed attributions. Do not turn analysis, significance, psychology, engagement guidance, or speculation into reported fact. Every concrete detail and attribution must be traceable to the verified facts or relevant source text supplied below. If a detail or attribution is not explicitly supported, leave it out.
...
content must be the complete article body. sections is metadata only and must not be used as a substitute for content. Include a lead section in sections when a lead exists. Do not include a title, byline, JSON fences, or meta commentary inside content.'''
        payload={"title":research["title"],"summary":research["summary"],"verified_facts":research["facts"],"sources":research["sources"],"editorial_context":research["context"],"grounding_rule":"Verified facts and relevant source text are the only factual authority. Editorial context is writing guidance only and must never introduce factual details not present in the verified facts or relevant sources."}
    ...
    def _expand_if_needed(self,client,research,result,attempt=1):
        if not isinstance(result,dict): return result
        content=self._text(result.get("content")); words=self._word_count(content)
        if words>=self.min_words: return result
        target=max(self.min_words+60,self.target_words)
        system=f"""You are the senior editor completing a serious digital news article.
The supplied draft is below the minimum publication length. This is a mandatory correction. Rewrite and expand it to AT LEAST {self.min_words} words; aim for {target} words when the supplied evidence supports it, and never exceed {self.max_words} words. The final content MUST NOT be below {self.min_words} words.
Use ONLY facts and information contained in the supplied research and draft. Do not invent names, quotes, statistics, dates, motives, locations, analysts, experts, officials, reactions, events, or background details. Do not create unnamed attributions or turn analysis, significance, psychology, engagement guidance, or speculation into reported fact.
Add useful verified context, chronology, consequences, significance, and story-specific detail where supported by the research. Develop the supplied facts rather than repeating them. Do not pad with generic filler.
...
This is expansion attempt {attempt}; if the previous draft was short, correct the length decisively while preserving factual grounding."""
        payload={"research":research,"draft":result,"instruction":f"Current draft is {words} words. Mandatory: return a complete article with at least {self.min_words} words. Do not return a short draft."}
        response=client.chat(messages=[{"role":"system","content":system},{"role":"user","content":json.dumps(payload,ensure_ascii=False)}],temperature=self.temperature,max_tokens=self.max_tokens,enable_thinking=False,response_format={"type":"json_object"})
        raw=self._strip_fences(self._response_text(response))
    ...
    def _quality_ok(self,article,research):
        ...
        if len(paragraphs)<5 or len(sentences)<10:
            if words<800: return False
        low=content.lower()
        ...
        if not self._grounded_content(content,research): return False
        return True

    def _relevant_sources(self,sources,title,story,facts):
        if not sources: return []
        target=self._tokens(title)
        target.update(self._tokens(self._text(story.get("description") or story.get("summary"))))
        target.update(self._tokens(" ".join(facts[:12])))
        entities=story.get("entities",{}) if isinstance(story,dict) else {}
        if isinstance(entities,dict):
            for value in entities.values():
                target.update(self._tokens(" ".join(self._text(x) for x in value) if isinstance(value,list) else value))
        ranked=[]
        for source in sources:
            blob=self._text(" ".join((source.get("title",""),source.get("text",""),source.get("name",""))))
            overlap=len(target & self._tokens(blob))
            exact=self._tokens(source.get("title",""))
            score=overlap+(6 if exact and len(exact & target)>=2 else 0)
            if score>=2: ranked.append((score,source))
        ranked.sort(key=lambda x:x[0],reverse=True)
        return [s for _,s in ranked[:10]]
    def _tokens(self,text):
        stop={"that","this","with","from","have","were","they","their","about","after","before","into","also","said","been","will","would","could","there","which","while","where","when","what","than","then"}
        return {w for w in re.findall(r"[a-z0-9]{4,}",self._clean_text(text).lower()) if w not in stop}
    def _evidence_sentences(self,research):
        out=[]
        for fact in research.get("facts",[]): out+=self._sentences(fact)
        for source in research.get("sources",[]): out+=self._sentences(source.get("text",""))
        return out
    def _grounded_content(self,content,research):
        evidence=" ".join(research.get("facts",[]))+" "+" ".join(s.get("text","") for s in research.get("sources",[]))
        evidence_low=evidence.lower()
        evidence_sentences=self._evidence_sentences(research)
        if not evidence.strip() or not evidence_sentences: return False
        for sentence in self._sentences(content):
            st=self._tokens(sentence)
            candidates=[e for e in evidence_sentences if len(st & self._tokens(e))>=2]
            nums=set(re.findall(r"\b\d+(?:[.:/-]\d+)*\b",sentence))
            if nums:
                if not candidates: return False
                if not any(nums.issubset(set(re.findall(r"\b\d+(?:[.:\/-]\d+)*\b",e))) for e in candidates): return False
            if re.search(r"\b(?:analysts?|experts?|officials?|researchers?|observers?|critics?|sources?|stakeholders?|industry figures?)\b",sentence,re.I):
                if not candidates: return False
                if not any(re.search(r"\b(?:analysts?|experts?|officials?|researchers?|observers?|critics?|sources?|stakeholders?|industry figures?)\b",e,re.I) for e in candidates): return False
        for quote in re.findall(r'[“"][^”"]{8,}[”"]',content):
            inner=self._clean_text(quote[1:-1]).lower()
            if inner and inner not in evidence_low and not any(len(self._tokens(inner)&self._tokens(e))>=max(3,len(self._tokens(inner))//2) for e in evidence_sentences): return False
        return True
