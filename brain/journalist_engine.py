...
Use ONLY the supplied research. Never invent facts, names, quotes, statistics, dates, motives, locations, background details, or events. Never manufacture quotations. Do not turn analysis or speculation into fact. Every concrete detail must be traceable to the verified facts or relevant source text supplied below. If a detail is not supported, leave it out.
...
content must be the complete rewritten article body."""
    ...
    def _quality_ok(self,article,research):
        ...
        if words>=800 and (len(paragraphs)<5 or len(sentences)<10):
            return False
        low=content.lower()
        ...
        if research["facts"] and not any(self._fact_overlap(f,content) for f in research["facts"]): return False
