...
class NvidiaBrainClient:
    def __init__(self,model:str="meta/llama-3.3-70b-instruct",base_url:str="https://integrate.api.nvidia.com/v1"):
        self.name="NVIDIA Brain Client";self.version="2.1.0";self.model=model;self.base_url=base_url.rstrip("/")
...
class BrainPipeline:
    ...
    def _extract_entities(self,story):
        ...
        if isinstance(entities,dict):return entities
        result={}
    ...
    def _build_provisional_article_plan(self,package):
        return {"status":"PROVISIONAL","topic":package.get("topic",""),"story":self._safe_dict(package.get("story")),"story_model":self._safe_dict(package.get("story_model")),"synthesis":self._safe_dict(package.get("synthesis")),"significance":self._safe_dict(package.get("significance")),"angles":self._safe_dict(package.get("angles")),"claims":package.get("claims",[]),"evidence":self._safe_dict(package.get("evidence")),"verification":self._safe_dict(package.get("verification")),"investigation":self._safe_dict(package.get("investigation")),"misinformation":self._safe_dict(package.get("misinformation")),"psychology":self._safe_dict(package.get("psychology")),"reader_psychology":self._safe_dict(package.get("reader_psychology")),"engagement":self._safe_dict(package.get("engagement")),"narrative":self._safe_dict(package.get("narrative")),"source_graph":self._safe_dict(package.get("source_graph")),"source_intelligence":self._safe_dict(package.get("source_intelligence")),"safe_claims":package.get("claims",[]),"excluded_claims":[],"article":{"headline":{},"dek":{},"lead":{},"key_facts":[],"context":{},"why_it_matters":{},"what_happens_next":{},"what_is_unknown":{},"sources":[]}}
    ...
    def _status_value(self,value):
        ...
        return "UNKNOWN"
    def _component_ready(self,component):return component is not None
