...
class NvidiaBrainClient:
    def __init__(
        ...
        try:
            self.timeout = int(os.getenv("NVIDIA_TIMEOUT", "180"))
...
class JournalistAIClient:
    """Use the NVIDIA client for article writing without duplicate reasoning."""
    def __init__(self, client: NvidiaBrainClient) -> None:
        self.client = client
...
class BrainPipeline:
    ...
    def _journalist_sources(self, sources: Any) -> List[Dict[str, Any]]:
        """Bound raw source text sent to the long-form writing request."""
    ...
    def run(
        ...
        story, freshness = self._select_fresh_story(story, sources)
        ...
        logger.info("=" * 70)
        logger.info(
            "STORY SELECTION: %s | published duplicates rejected: %s | candidates: %s",
            freshness.get("selected_title", "NONE"),
            freshness.get("duplicates_rejected", 0),
            freshness.get("candidate_count", 0),
        )

        if not story:
            package["pipeline_status"] = "NO_FRESH_STORY"
            package["status"] = "NO_FRESH_STORY"
            package["publication_ready"] = False
            package["brain_summary"] = {
                "status": "NO_FRESH_STORY",
                "topic": topic,
            ...
            logger.warning("No fresh unpublished story available; publication blocked.")
            return package
        ...
        try:
            ...
            package["cluster"] = self._safe(
                ...
                {
                    "stories": sources,
                    "sources": self._journalist_sources(sources),
                    "story": package["story"],
                },
            )
            ...
            final_plan = self._safe(
                ...
                ],
                {
                    ...
                    "engagement": package["engagement"],
                    "significance": package["significance"],
                    "ai": JournalistAIClient(self.ai),
                },
            )

            package["journalism"] = final_plan if isinstance(final_plan, dict) else {}
            ...
            journalism = package.get("journalism", {})
            journalism_status = str(
                journalism.get("status", "")
                if isinstance(journalism, dict)
                else ""
            ).strip().upper()
            journalism_ok = journalism_status == "JOURNALISM_COMPLETE"
            journalism_safe = (
                journalism.get("publication_safe", True)
                if isinstance(journalism, dict)
                else False
            )
            package["publication_ready"] = bool(
                journalism_ok
                and journalism_safe is not False
                and self._editor_allows_publication(
                    package["editorial"]
                )
            )
            ...
            package["brain_summary"] = {
                ...
                "journalism": journalism_status or "UNKNOWN",
    ...
    def _select_fresh_story(
        self,
        story: Dict[str, Any],
        sources: List[Dict[str, Any]],
    ):
        published = self._load_published_articles()
        published_keys = self._published_keys(published)
        candidates = []
        duplicates = 0

        for item in sources:
            candidate = dict(item)
            if not self._story_has_identity(candidate):
                continue
            keyset = self._story_keys(candidate)
            if keyset & published_keys:
                duplicates += 1
                continue
            candidate["_freshness_score"] = self._freshness_score(candidate)
            candidates.append(candidate)

        selected = None
        supplied_duplicate = False
        if story and self._story_has_identity(story):
            if self._story_keys(story) & published_keys:
                supplied_duplicate = True
            else:
                selected = dict(story)
                selected.pop("_freshness_score", None)

        if selected is None and candidates and not story:
            candidates.sort(key=lambda x: x.get("_freshness_score", 0), reverse=True)
        ...
        info = {
            ...
            "supplied_story_duplicate": supplied_duplicate,
            "published_article_count": len(published),
            "selected": bool(selected),
            "selected_title": str((selected or {}).get("title") or (selected or {}).get("headline") or "").strip(),
            "selected_source_url": str((selected or {}).get("source_url") or (selected or {}).get("url") or "").strip(),
        }
        return selected or {}, info

    def _load_published_articles(self) -> List[Dict[str, Any]]:
        urls = []
        repo = os.getenv("GITHUB_REPOSITORY", "").strip().strip("/")
        branch = os.getenv("GITHUB_BRANCH", "main").strip() or "main"
        if repo:
            urls.append(f"https://raw.githubusercontent.com/{repo}/{branch}/data/articles.json")
        site = os.getenv("GITHUB_SITE_URL", "").strip().rstrip("/")
        if site:
            urls.append(site + "/data/articles.json")

        for url in urls:
            try:
                response = requests.get(
                    url + ("&" if "?" in url else "?") + "v=" + str(int(__import__("time").time())),
                ...
                logger.warning("Published article index unavailable from %s: %s", url, exc)

        return []

    def _published_keys(self, articles: List[Dict[str, Any]]) -> set:
        keys = set()
        for article in articles:
            keys.update(self._story_keys(article))
        return keys

    def _story_keys(self, item: Dict[str, Any]) -> set:
        keys = set()
        ...
        slug = self._normalize_identity(item.get("slug") or "")
    ...
    def _story_has_identity(self, item: Dict[str, Any]) -> bool:
        return bool(self._story_keys(item))
    ...
    def _freshness_score(self, item: Dict[str, Any]) -> float:
        ...
        value = item.get("published_at") or item.get("published") or item.get("date") or item.get("pub_date") or ""
        score = 0.0
        if value:
            try:
                ...
                age_hours = max(0.0, (dt.datetime.now(dt.timezone.utc) - parsed.astimezone(dt.timezone.utc)).total_seconds() / 3600.0)
                score += max(0.0, 100000.0 - age_hours * 1000.0)
            except Exception:
                pass
        title = str(item.get("title") or item.get("headline") or "").strip()
        description = str(item.get("description") or item.get("summary") or item.get("content") or item.get("text") or "").strip()
        source_url = str(item.get("source_url") or item.get("url") or "").strip()
        score += min(len(description), 5000) * 0.5
        score += min(len(title), 180) * 2.0
        score += 500.0 if source_url else 0.0
        return score
    ...
    def _extract_article(
        ...
        for candidate in candidates:
            if not isinstance(candidate, dict):
                continue
            title = candidate.get("title") or candidate.get("headline") or ""
            content = candidate.get("content") or candidate.get("body") or candidate.get("article") or candidate.get("text") or ""
            if title or content:
                article = dict(candidate)
                article["title"] = str(title)
                article["headline"] = str(candidate.get("headline") or title)
                article["content"] = str(content)
                article["body"] = str(content)
                article.setdefault("dek", "")
                article.setdefault("lead", "")
                article.setdefault("key_facts", [])
                article.setdefault("context", [])
                article.setdefault("why_it_matters", "")
                article.setdefault("what_happens_next", "")
                article.setdefault("what_is_unknown", "")
                article.setdefault("sources", [])
                article.setdefault("seo_title", str(title))
                article.setdefault("seo_description", "")
                article.setdefault("slug", "")
                return article

        return {
            "title": str(article_plan.get("title") or article_plan.get("headline") or ""),
            "headline": str(article_plan.get("headline") or article_plan.get("title") or ""),
            "content": str(article_plan.get("content") or article_plan.get("body") or ""),
            "body": str(article_plan.get("content") or article_plan.get("body") or ""),
        }
