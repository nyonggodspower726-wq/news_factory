...
class MediaManager:
    ...
    def attach(self,article:Dict[str,Any],story:Dict[str,Any],generate_image:bool=True,platform:str="website")->Dict[str,Any]:
        ...
        updated["image_url"]=self._clean(media.get("image_url",""))
        updated["image_alt"]=self._clean(media.get("alt",""))
        updated["image_credit"]=self._clean(media.get("credit",""))
        updated["image_source_url"]=self._clean(media.get("source_url",""))
        updated["image_caption"]=self._clean(media.get("caption",""))
        updated["image_source_type"]=self._clean(media.get("source_type",""))
        updated["image_local_path"]=self._clean(media.get("local_path",""))
        updated["has_image"]=bool(media.get("has_image",False))

        updated["media_status"]=prepared.get("status","")
        updated["media_source_type"]=prepared.get("source_type","")

        if prepared.get("generation"):
            updated["image_generation"]={
                "status":prepared["generation"].get("status",""),
                "generated":prepared["generation"].get("generated",False),
                "story_type":prepared["generation"].get("story_type",""),
                "platform":prepared["generation"].get("platform",platform)
            }
...
def create_media_manager(image_engine:Optional[ImageEngine]=None,image_generator:Optional[ImageGenerator]=None)->MediaManager:
    return MediaManager(
