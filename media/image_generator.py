...
class ImageGenerator:
    ...
    def generate(self,article,platform="website",mode="auto",output_format="png",width=1280,height=720):
        ...
        try:
            ...
            return {"status":"IMAGE_READY","generated":True,"image_url":self._public_url(path),"local_path":str(path),"prompt":prompt,"story_type":story_type,"platform":platform,"width":width,"height":height,"alt_text":self.alt(article),"caption":self.caption(article),"credit":"AI-generated editorial illustration"}
    ...
    def build_prompt(self,article):
        ...
        return self._clean(f"Professional editorial news photography, realistic documentary style. Create an original non-deceptive image illustrating this verified news story. Topic: {topic}. Headline: {title}. Context: {summary}. Location: {location}. Story type: {style}. No fake evidence, fake documents, misleading proof, or graphic content.")
    ...
    def generate_prompt_only(self,article):
        return {"status":"PROMPT_READY","prompt":self.build_prompt(article),"story_type":self.story_type(article),"alt_text":self.alt(article),"caption":self.caption(article)}
