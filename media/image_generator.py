...
class ImageGenerator:
    ...
    def build_prompt(self,article):
        ...
        prompt=(
            ...
            f"Use realistic documentary photography, sophisticated photojournalism, cinematic composition, "
            f"natural lighting, realistic people and environments, authentic visual details, strong depth, "
            f"and a polished professional newsroom aesthetic. "
            f"Create an original visual scene that represents the subject of this verified news story. "
            f"Visual subject: {title}. "
            f"Topic: {topic}. "
            f"Story context for visual interpretation only: {summary}. "
            f"Location: {location}. "
            f"Story type: {style}. "
            f"The image must communicate the story entirely through visual storytelling. "
            f"IMPORTANT ABSOLUTE RULE: DO NOT GENERATE ANY TEXT INSIDE THE IMAGE. "
            f"ZERO TEXT. ZERO LETTERS. ZERO WORDS. ZERO NUMBERS. "
            f"Do not generate headlines, captions, subtitles, newspaper articles, newspaper pages, "
            f"magazines, books, documents, reports, websites, social media posts, advertisements, "
            f"logos, trademarks, watermarks, labels, signs, posters, banners, badges, charts with text, "
            f"computer interfaces, phone interfaces, television text, presentation slides, readable screens, "
            f"license plates, name tags, product labels, or any other written language. "
            f"Do not place the story headline or article title anywhere in the image. "
            f"Do not simulate a newspaper or news website. "
            f"If screens, phones, computers, televisions, papers, signs or documents appear naturally in the scene, "
            f"they must be blank, blurred, abstract, or positioned so that no writing is visible. "
            f"Prefer clean visual environments without text-bearing objects. "
            f"No typography of any kind. "
            f"No fake evidence, fake documents, misleading proof, fabricated screenshots, or deceptive visual claims. "
            f"The final image must look like a professional editorial photograph selected by a major international newsroom."
        )
    ...
    def _headers(self):
        return {
            ...
            "User-Agent":"AI-News-Factory/1.4"
        }
