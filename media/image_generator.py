...
class ImageGenerator:
        ...
        try:
            ...
            if content_type.startswith("image/") or self._looks_like_image(response.content):
                ...
                try:
                    ...
                    logger.error("Image API returned an unsupported response. Content-Type=%s, length=%s",content_type,len(response.content))
    ...
    def build_prompt(self,article: Dict[str,Any])->str:
        ...
        context_part=f"Context: {summary}. " if summary else ""
        location_part=f"Location: {location}. " if location else ""
        prompt=(
            f"{style}. "
            "Create an original editorial image illustrating the verified news topic. "
            f"Topic: {topic}. "
            f"Event: {event}. "
            f"Headline: {title}. "
            f"{context_part}"
            f"{location_part}"
            "The image must be visually relevant, realistic, non-deceptive, and non-graphic unless the subject itself requires otherwise. "
            "Do not create fabricated logos, fake documents, fake evidence, misleading text, or deceptive visual proof. "
            "Do not include readable text, captions, headlines, signs, watermarks, or artificial typography in the image."
        )
    ...
    def _headers(self)->Dict[str,str]:
        return {
            ...
            "User-Agent":"AI-News-Factory/1.2"
        }
    ...
    def _save(self,image: Any,article: Dict[str,Any],output_format: str)->Path:
        ...
        slug=re.sub(r"[^a-z0-9]+","-",title).strip("-")[:70]
        if not slug:
            slug="news"
        ...
        if isinstance(image,(bytes,bytearray)):
            ...
            response=requests.get(image,timeout=self.timeout,headers={"User-Agent":"AI-News-Factory/1.2"})
            response.raise_for_status()
    ...
    def _detect_extension(self,image: Any,requested_format: str)->str:
        if isinstance(image,(bytes,bytearray)):
            ...
            if len(data)>=12 and data[:4]==b"RIFF" and data[8:12]==b"WEBP":
                return "webp"
        requested=(requested_format or "png").lower()
    ...
    def status(self)->Dict[str,Any]:
        return {
            ...
            "credential_source":"CLOUDFLARE_API_TOKEN" if os.getenv("CLOUDFLARE_API_TOKEN","").strip() else ("IMAGE_API_KEY" if os.getenv("IMAGE_API_KEY","").strip() else "NONE")
        }
